...
<!--<nav class="navbar navbar-expand-lg navbar-dark bg-secondary">
  ...
  $result = mysqli_query($link, $query);
      ...
      echo '<div  class ="form" id="form">';
			...
			echo '<thead ><tr ><th>#</th> <th>Name</th><th>Last Name</th><th>Major</th><th>Phone Number</th><th>Email</th><th>Delete</th></tr></thead>';
			while ($row = mysqli_fetch_assoc($result)){
				echo '<tr><td>' . $row['id'] . '</td><td>' . $row['Name'] . '</td><td>' . $row['Last_Name'] . '</td><td>' . $row['Major'] . '</td><td>' . $row['Phone']. '</td><td>' . $row['Email']. '</td>'
        .'<td><a href="drop.php?id='.$row['id'].'">	<button class="btn btn-light btn-outline-dark" type="button" id="submit">Delete</button>';
	...
	?>
	  ...
	  <!--<div class="d-grid gap-2 d-md-flex justify-content-md-end">
    	<a href="itPage.php"><button class="btn btn-light btn-outline-secondary me-md-2" type="button">Home</button></a>
  	</div>-->
